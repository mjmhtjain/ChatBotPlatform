...
interface Props {
  projectId: string
  onClose: () => void
  onCreated: (flow: FlowMeta) => void
}

export default function NewFlowModal({ projectId, onClose, onCreated }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">New Flow</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="flow-name" className="block text-sm font-medium text-gray-700 mb-1.5">
              Flow name
            </label>
            <input
              id="flow-name"
